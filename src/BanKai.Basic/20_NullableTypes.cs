...
namespace BanKai.Basic
{
    ...
    public class NullableTypes
    {
        ...
        public void should_never_be_null_for_value_type()
        {
            ...
            bool containsSyntaxError = compilerResults.Errors.Count > 0;  //值类型不能为null
        ...
        {
            Type nullableType = typeof(Nullable<int>);  //Nullable:表示可以分配Null的值类型
        ...
        public void should_use_has_value_to_determine_if_a_nullable_type_has_value_type_instance()
        {
            ...
            bool hasValueForWithoutValue = nullableIntWithoutValue.HasValue; //判断可空类型中是否有值类型实例
        ...
        public void should_bring_operators_for_underlaying_type()
        {
            int? larger = 5;
            int? smaller = 1;
            //(larger==null || smaller == null) ? false : larger.Value > smaller.Value
            // change the variable value to fix the test.
